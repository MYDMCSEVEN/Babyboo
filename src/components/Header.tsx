    ...
    <header className="bg-white shadow-sm sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* Mobile menu button */}
          <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden text-baby-text">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              {menuOpen ? (
        ...
        {menuOpen && (
          <nav className="md:hidden pb-4 space-y-2">
            <Link href="/boutique" className="block py-2 text-baby-text hover:text-baby-brown" onClick={() => setMenuOpen(false)}>
              Boutique
            </Link>
            <Link href="/notre-histoire" className="block py-2 text-baby-text hover:text-baby-brown" onClick={() => setMenuOpen(false)}>
              Notre Histoire
            </Link>
            <Link href="/contact" className="block py-2 text-baby-text hover:text-baby-brown" onClick={() => setMenuOpen(false)}>
              Contact
            </Link>
            <Link href="/panier" className="block py-2 text-baby-text hover:text-baby-brown" onClick={() => setMenuOpen(false)}>
              Panier ({cartCount})
            </Link>
          </nav>
        )}
